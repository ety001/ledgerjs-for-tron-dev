...
exports.signTIP712Message = exports.signTIP712HashedMessage = void 0;
const utils_1 = require("../utils");
const trc20_1 = require("../services/trc20");
const loadConfig_1 = require("../services/loadConfig");
...
const makeRecursiveFieldStructImplem = ({ transport, loadConfig, chainId, erc20SignaturesBlob, types, filters, shouldUseV1Filters, shouldUseDiscardedFields, coinRefsTokensMap, }) => {
    ...
    const recursiveFieldStructImplem = async (destructedType, data, path = "") => {
        ...
        const isCustomType = !utils_2.TIP712_TYPE_PROPERTIES[typeDescription?.name?.toUpperCase() || ""];
...
const sendStructImplem = async (transport, structImplem) => {
    ...
    if (structType === "array") {
        return transport.send(APDU_FIELDS.CLA, APDU_FIELDS.INS, APDU_FIELDS.P1_complete, APDU_FIELDS.P2_array, Buffer.from((0, utils_2.intAsHexBytes)(value, 1), "hex"));
    }
    if (structType === "field") {
        const { data: rawData, type, sizeInBits } = value;
        const encodedData = utils_2.TIP712_TYPE_ENCODERS[type.toUpperCase()]?.(rawData, sizeInBits);
        if (encodedData) {
            ...
            const data = Buffer.concat([
                Buffer.from((0, utils_2.intAsHexBytes)(dataLengthPer16Bits, 1), "hex"),
                Buffer.from((0, utils_2.intAsHexBytes)(dataLengthModulo16Bits, 1), "hex"),
                encodedData,
            ]);
...
async function sendFilteringInfo(transport, type, loadConfig, data) {
    ...
    switch (type) {
        ...
        case "contractName": {
            ...
            const filtersCountBuffer = Buffer.from((0, utils_2.intAsHexBytes)(filtersCount, 1), "hex");
        ...
        case "showField": {
            ...
            if (isTokenAddress && coinRef !== undefined) {
                ...
                if (deviceTokenIndex === undefined) {
                    const payload = await (0, trc20_1.byContractAddressAndChainId)(token, chainId, erc20SignaturesBlob);
                    if (payload) {
        ...
        case "discardField": {
            ...
            const pathLengthBuffer = Buffer.from((0, utils_2.intAsHexBytes)(pathBuffer.length, 1), "hex");
...
const signTIP712Message = async (transport, path, typedMessage, fullImplem = false, loadConfig) => {
    ...
    const erc20SignaturesBlob = !shouldUseV1Filters
        ? await (0, trc20_1.findERC20SignaturesInfo)(loadConfig, domain.chainId || 0)
        : undefined;
...
exports.signTIP712Message = signTIP712Message;
