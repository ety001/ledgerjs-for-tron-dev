...
import { getFiltersForMessage, sortObjectAlphabetically, getCoinRefTokensMap, makeTypeEntryStructBuffer, intAsHexBytes, getFilterDisplayNameAndSigBuffers, getPayloadForFilterV2, TIP712_TYPE_ENCODERS, destructTypeFromString, TIP712_TYPE_PROPERTIES, } from "./utils";
const CLA = 0xe0;
...
const sendStructImplem = async (transport, structImplem) => {
    ...
    if (structType === "field") {
        ...
        const encodedData = TIP712_TYPE_ENCODERS[type.toUpperCase()]?.(rawData, sizeInBits);
        if (encodedData) {
